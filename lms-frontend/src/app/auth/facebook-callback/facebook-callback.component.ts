...
export class FacebookCallbackComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (error) {
      this.error = 'Đăng nhập Facebook bị hủy hoặc thất bại';
    ...
    this.oauth2Service.facebookCallback(code).subscribe({
      next: (response) => {
      ...
      error: (err) => {
        console.error('Facebook OAuth2 callback error:', err);
        this.error = err.error?.message || 'Đăng nhập Facebook thất bại';
        this.toastr.error(this.error || 'Đăng nhập Facebook thất bại');
        this.loading = false;
